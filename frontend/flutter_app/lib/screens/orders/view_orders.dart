...
class _ViewOrdersState extends State<ViewOrders> {
  ...
  // List<TableItemData> _penddingOrders = [];
  // List<TableItemData> _completedOrders = [];
  // List<TableItemData> _cancelledOrders = [];
  ...
  Future<void> _fetchOrders() async {
    try {
      ...
      final response = await http.get(
        Uri.parse('${AppConstants.kApiHostSpring}/api/orders/history'),
        headers: {
  ...
  void _processOrdersToTables() {
    ...
    for (var order in _allOrders) {
      ...
      final itemData = TableItemData(
        ...
        // TODO make view order detail page
        onTap: (context) {
          NavigationUtils.push(context, ViewOrderDetail(order: order));
        },
      );
