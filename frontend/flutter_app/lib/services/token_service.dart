...
const String _authBaseUrl = 'https://lauderdale-surround-lender-forwarding.trycloudflare.com/api/auth';
...
class TokenService {
  ...
  Future<void> saveTokens(String accessToken, String refreshToken, int expiresInSeconds) async {
    final expiryTime = DateTime.now().add(Duration(seconds: expiresInSeconds));
    
    await _storage.write(key: _accessTokenKey, value: accessToken);
    await _storage.write(key: _refreshTokenKey, value: refreshToken);
    await _storage.write(key: _accessTokenExpiryKey, value: expiryTime.millisecondsSinceEpoch.toString());
  ...
  Future<String?> getAccessToken() async {
    ...
    if (expiry.subtract(buffer).isBefore(DateTime.now())) {
      if (kDebugMode) {
        print("Access token is near expiration or expired. Attempting proactive refresh...");
      }
